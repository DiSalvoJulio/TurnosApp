...
import { User, LogOut, ChevronDown, Calendar, Clock, LayoutDashboard, ClipboardList } from 'lucide-react';
...
export default function ProfessionalLayout({ children }: ProfessionalLayoutProps) {
    ...
    const navItems = [
        { label: 'Panel Principal', icon: LayoutDashboard, path: '/professional/dashboard' },
        { label: 'Mis Turnos', icon: Calendar, path: '/professional/agenda' },
        { label: 'Mis Horarios', icon: Clock, path: '/professional/schedule' },
        { label: 'Historias Clínicas', icon: ClipboardList, path: '/professional/history' },
        { label: 'Listados', icon: ClipboardList, path: '/professional/listados' },
    ];

    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            {/* Header */}
            <header className="print:hidden bg-blue-50/80 backdrop-blur-xl border-b border-blue-100 px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm shadow-blue-500/5">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                        <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100">
                            <Calendar className="w-6 h-6 text-white" />
                        </div>
                        <span className="text-xl font-black text-slate-800 tracking-tight hidden md:block">TurnosPRO</span>
                    </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden md:block text-right mr-2">
                        ...
                        <h1 className="text-sm font-black text-slate-900 leading-none mt-0.5">Dr. {firstName}</h1>
                    </div>
                    ...
                    <div className="relative" ref={dropdownRef}>
                        ...
                        {isProfileOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 animate-in fade-in zoom-in-95 duration-100 ring-1 ring-black/5">
                                ...
                                <div className="border-t border-slate-50 my-1 lg:hidden"></div>
                                <Link
