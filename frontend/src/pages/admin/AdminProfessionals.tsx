...
import { Eye, EyeOff, ArrowLeft, Mail, User, Fingerprint, Phone, MapPin, Calendar, Activity } from 'lucide-react';
...
const emptyForm: Omit<ProfessionalRow, 'id' | 'isActive'> & { password: string } = {
    firstName: '',
    lastName: '',
    specialty: '',
    phone: '',
...
export default function AdminProfessionals() {
    ...
    const [sortKey, setSortKey] = useState<'name' | 'specialty' | 'email'>('name');
    const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc');
    const [professionsList, setProfessionsList] = useState<{ id: string, name: string, isActive: boolean }[]>([]);
    ...
    const startEdit = (prof: ProfessionalRow) => {
        setIsCreateOpen(false);
        ...
        setForm({
            ...
            email: prof.email,
            dni: prof.dni,
            dateOfBirth: prof.dateOfBirth ? prof.dateOfBirth.substring(0, 10) : '',
    ...
    const toggleActive = async (prof: ProfessionalRow) => {
        try {
            await api.patch(`/users/professional/${prof.id}/status`, { isActive: !prof.isActive });
            Swal.fire({
                title: prof.isActive ? 'Desactivado' : 'Activado',
                icon: 'success',
                timer: 1500,
    ...
    const handleDelete = async (prof: ProfessionalRow) => {
        const result = await Swal.fire({
            title: '¿Estás seguro?',
            text: `Se eliminará permanentemente al profesional "${prof.firstName} ${prof.lastName}". Esta acción no se puede deshacer.`,
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                await api.delete(`/users/${prof.id}`);
                Swal.fire('¡Eliminado!', 'El profesional ha sido borrado.', 'success');
                loadProfs();
            } catch {
                Swal.fire('Error', 'No se pudo eliminar al profesional.', 'error');
            }
        }
    };
    ...
    }, [profs, search, sortKey, sortOrder]);

    const exportCsv = () => {
        const rows = ['Nombre,DNI,Especialidad,Teléfono,Email,Estado'];
        sortedFiltered.forEach((p) => {
            rows.push(`"${p.firstName} ${p.lastName}","${p.dni}","${p.specialty}","${p.phone}","${p.email}","${p.isActive ? 'Activo' : 'Inactivo'}"`);
        });
        const blob = new Blob([rows.join('\n')], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = 'profesionales.csv';
        link.click();
        ...
        <div className="max-w-6xl mx-auto p-4 bg-white rounded-3xl shadow-sm border border-slate-100">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900">Gestión de Profesionales</h1>
            ...
            {isCreateOpen && (
                <div className="border border-slate-200 rounded-3xl p-6 bg-slate-50 mb-6 shadow-inner animate-in slide-in-from-top-4 duration-300">
                    <h2 className="text-xl font-bold mb-4 text-indigo-900">Crear nuevo profesional</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-slate-600 ml-1">Nombre</label>
                            <input name="firstName" value={form.firstName} onChange={updateForm} placeholder="Nombre" className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-slate-600 ml-1">Apellido</label>
                            <input name="lastName" value={form.lastName} onChange={updateForm} placeholder="Apellido" className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-slate-600 ml-1">DNI</label>
                            <input name="dni" value={form.dni} onChange={updateForm} placeholder="DNI" className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-slate-600 ml-1">Fecha de Nacimiento</label>
                            <input name="dateOfBirth" type="date" value={form.dateOfBirth} onChange={updateForm} className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                            ...
                            <input name="phone" value={form.phone} onChange={updateForm} placeholder="Teléfono" className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-slate-600 ml-1">Dirección</label>
                            <input name="address" value={form.address} onChange={updateForm} placeholder="Dirección" className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                        ...
                        <div className="space-y-1 md:col-span-2">
                            ...
                            <input name="email" value={form.email} onChange={updateForm} placeholder="Email" className="border p-2.5 rounded-xl w-full focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium" />
                        </div>
                    </div>
                    <div className="flex gap-2 mt-6">
                        <button onClick={createProfessional} className="bg-green-600 hover:bg-green-700 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-green-100 transition-all active:scale-95">Guardar profesional</button>
            ...
            <div className="flex flex-wrap gap-2 items-center mb-5">
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar nombre o especialidad..." className="border border-slate-200 p-2.5 rounded-xl w-full md:w-72 focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
                <select value={sortKey} onChange={(e) => setSortKey(e.target.value as 'name' | 'specialty' | 'email')} className="border border-slate-200 p-2.5 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium text-slate-600">
            ...
            {loading ? (
                <div className="flex flex-col items-center py-12">
                    ...
                    <p className="text-slate-400 font-medium">Cargando profesionales...</p>
                </div>
            ) : (
                                ...
                                {sortedFiltered.map((p) => (
                                    <tr key={p.id} className="border-t border-slate-50 hover:bg-indigo-50/20 transition-colors group">
                                        ...
                                        <td className="p-4 text-xs">
                                            <span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded-lg font-bold">{p.specialty}</span>
                                        </td>
                        ...
                        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
                            <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 w-full max-w-2xl overflow-hidden animate-in zoom-in duration-300">
                                <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                                    <div>
                                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Editar profesional</h2>
                                        <p className="text-slate-500 font-medium text-sm">Actualice los datos del especialista</p>
                                    </div>
