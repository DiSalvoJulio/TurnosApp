...
export default function ProfessionalProfile() {
    const [profile, setProfile] = useState<Profile>({
        firstName: '', lastName: '', dni: '', dateOfBirth: '', address: '',
    ...
    useEffect(() => {
        api.get(`/users/professional/${profId}/profile`)
            .then(r => {
                const data = r.data;
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const requiredFields: (keyof Profile)[] = ['firstName', 'lastName', 'address', 'phone', 'dateOfBirth'];

        requiredFields.forEach(f => {
            if (!profile[f as keyof Profile] || profile[f as keyof Profile]?.toString().trim() === '') {
        ...
        try {
            await api.put(`/users/professional/${profId}/profile`, {
                ...profile,
    ...
    const field = (label: string, name: keyof Profile | 'confirmPassword', icon: React.ComponentType<{ className?: string }>, type = 'text', readOnly = false) => {
        const Icon = icon;
        ...
        return (
            <div className="space-y-2">
                ...
                <div className="relative group">
                    <div className={`absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-blue-600 transition-colors ${hasError ? 'text-red-500' : 'text-slate-400'}`}>
                    ...
                    <input
                        ...
                        name={name}
                        readOnly={readOnly}
                        placeholder={isPassword ? 'Ingrese contraseña' : ''}
                        className={`w-full pl-12 pr-12 py-4 bg-slate-50 border rounded-2xl focus:bg-white focus:ring-4 outline-none text-lg transition-all font-medium 
                            ...
                            ${hasError ? 'border-red-500 focus:ring-red-50 focus:border-red-500 bg-red-50/10' : 'border-slate-100 focus:ring-blue-50 focus:border-blue-500'}`}
        ...
        <div className="max-w-4xl mx-auto p-6 md:p-12 pb-24">
            ...
            {loading ? (
                ...
                <form onSubmit={handleSave} className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-slate-100 relative overflow-hidden">
                    ...
                    {saved && (
                        <div className="p-4 mb-8 bg-blue-50 border border-blue-100 text-blue-700 rounded-2xl font-bold text-sm flex items-center gap-3 animate-in fade-in slide-in-from-top-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                        ...
                        <div>
                            {field('Especialidad', 'specialty', Activity, 'text', true)}
                            <p className="mt-2 ml-1 text-xs text-slate-400 font-medium italic">* Solo modificable por administración.</p>
                        </div>
                        <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-50">
                        {!isEditing ? (
                            <Button
                                ...
                                className="flex-1 rounded-2xl py-4 text-lg font-black bg-blue-600 hover:bg-blue-700 shadow-xl shadow-blue-100 transition-all active:scale-95 flex items-center justify-center gap-2 text-white"
                            >
                            ...
                            <>
                                ...
                                <Button
                                    disabled={saving}
                                    className="flex-[2] rounded-2xl py-4 text-lg font-black bg-blue-600 hover:bg-blue-700 shadow-xl shadow-blue-100 transition-all active:scale-95 flex items-center justify-center gap-2 text-white"
                                >
